...
/** Legacy login/steps "ქულა" — retired. Daily claim advances streak only. */
export const DAILY_LOGIN_POINTS = 0;
/** @deprecated Steps goals no longer award User.points (Medi Coins via Quest). */
export const STEPS_GOAL_POINTS = 0;
  ...
  try {
    await prisma.$transaction(async (tx) => {
      await tx.dailyCheckIn.create({
        data: { userId, date: todayDate, pointsAwarded: 0 },
      });
...
/**
 * Legacy steps-goal +3 points — retired.
 * Endpoint kept so old clients flush queues safely without writing User.points.
 */
