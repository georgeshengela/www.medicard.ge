/**
 * Cycle Phase 31 — observation explainability QA seed.
 *
 *   node scripts/cycle-phase31-qa-seed.js pregnancy-none|pregnancy-higher|pregnancy-numbers|pregnancy-short|peri-higher|peri-lower|peri-numbers|peri-imbalance
 *
...
async function main() {
  ...
  const days = new Map();
  if (FIXTURE === 'pregnancy-none' || FIXTURE === 'peri-none') {
    present(days, today, peri ? 'hot_flashes' : 'nausea', [0, -6, -12, -18, -24]);
  ...
  } else if (FIXTURE === 'peri-lower') {
    present(days, today, 'hot_flashes', [-1, -3, -15, -17, -19, -21, -23, -25]);
    absent(days, today, 'hot_flashes', [0, -2, -4, -5, -6, -7, -8, -9, -14, -16, -18, -20]);
  ...
  console.log(`Phase 31 seeded ${FIXTURE} for ${EMAIL} today=${today} logs=${days.size} short=${shortEpisode}`);
}
