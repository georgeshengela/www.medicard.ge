/**
 * Cycle Phase 34 — owner-opted prenatal care calendar export QA seed.
 *
 *   node scripts/cycle-phase34-qa-seed.js none|on|past|completed
 *
 * Account: cycle.qa.phase6@medicard.ge / CycleQaPhase6a
 *
 * `none` = future plannedDate, reminder OFF, private note present (must not leak into OS event).
 */
...
async function main() {
  ...
  if (FIXTURE === 'none' || FIXTURE === 'on' || FIXTURE === 'past') {
    states.push({
      ...
      status: 'PLANNED',
      plannedDate: FIXTURE === 'past' ? addDays(today, -2) : addDays(today, 8),
    ...
    JSON.stringify(
      {
        ...
        today,
        plannedDate: states[0]?.plannedDate || null,
        reminderEnabled: Boolean(states[0]?.reminderEnabled),
      },
      null,
      2,
    ),
