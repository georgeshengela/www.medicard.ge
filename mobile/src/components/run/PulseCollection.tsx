...
export function PulseCollection({ view }: { view: ReturnType<typeof usePulse> }) {
  ...
  return <>
    <Card style={{ gap: 16, borderRadius: 27 }}>
    ...
    <View accessibilityRole="tablist" style={{ flexDirection: 'row', gap: 7 }}>
      {tabs.map(tab => <Pressable key={tab.id} accessibilityRole="tab" accessibilityLabel={`${tab.label}: ${tab.count}`} accessibilityState={{ selected: section === tab.id }} onPress={() => setSection(tab.id)} style={{ flex: 1, minWidth: 0, paddingVertical: 12, paddingHorizontal: 3, borderRadius: 19, borderWidth: 1, borderColor: section === tab.id ? '#14B8A6' : c.bg300, backgroundColor: section === tab.id ? c.accent100 : c.surface, alignItems: 'center', gap: 7 }}><View style={{ flexDirection: 'row', alignItems: 'center', gap: 7 }}><tab.icon size={17} color={c.primary100} /><Copy bold size={14}>{tab.count}</Copy></View><Copy size={10} bold>{tab.label}</Copy></Pressable>)}
    </View>

    {section === 'walks' ? <View style={{ gap: 11 }}>
      {!walks.length ? <Card style={{ padding: 24, gap: 12 }}><Footprints color={c.primary100} size={28} /><Copy bold size={18}>პირველი გზა წინ არის</Copy><Copy muted>დაიწყე გასეირნება იქ, სადაც ხარ. დასრულების შემდეგ შენი მანძილი და დრო აქ გამოჩნდება.</Copy></Card> : walks.map(session => <Card key={session.id} style={{ padding: 16, borderRadius: 22, gap: 12 }}>
