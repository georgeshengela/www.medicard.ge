...
export default function PulseActive(){
 ...
 return <View style={{flex:1,backgroundColor:c.bg100}}>
  ...
  <View pointerEvents="box-none" style={{position:'absolute',top:insets.top+8,left:14,right:14,gap:8}}>
   <View style={{flexDirection:'row',alignItems:'center',gap:8}}><IconButton label="უკან" icon={ArrowLeft} onPress={leave}/><Card style={{flex:1,paddingVertical:8,paddingHorizontal:13,borderRadius:18,gap:0}}><Copy bold size={13}>MEDI RUN <Copy muted size={9}>· პულსი</Copy></Copy><Copy size={10} muted>{running?(run.accuracyM!=null&&run.accuracyM<=25?'GPS აქტიურია · გზა ინახება':'ზუსტ GPS-ს ველოდებით'):run.phase==='paused'?'პაუზა · ჩანაწერი შენარჩუნებულია':'თავისუფლება შენს ნაბიჯებში'}</Copy></Card><IconButton label="მენიუ" icon={MoreHorizontal} onPress={()=>setMenu(true)}/></View>
   {mission?<Pressable accessibilityRole="button" onPress={()=>setPanel('missions')} style={{alignSelf:'flex-start',backgroundColor:c.surface,borderRadius:13,paddingVertical:7,paddingHorizontal:12,borderWidth:1,borderColor:c.bg300}}><Copy size={11} bold>{mission.name} · {missionPercent(pulse.book,mission)}%</Copy></Pressable>:null}
  ...
  {center?<View pointerEvents="box-none" style={{position:'absolute',bottom:insets.bottom+164,right:14,alignItems:'flex-end',gap:9}}><IconButton label="ჩემს მდებარეობაზე დაბრუნება" icon={LocateFixed} active={following} onPress={()=>map.current?.send({type:'follow'})}/></View>:null}
  {center?<View style={{position:'absolute',bottom:Math.max(12,insets.bottom),left:14,right:14,gap:10}}>
   {running&&pulse.signal.signal?<Pressable accessibilityRole="button" accessibilityLabel={pulse.signal.revealed?'საჩუქრის აღმოჩენა':'გულისცემის სიგნალი'} onPress={()=>{if(pulse.signal.revealed)setGift(true);else setNotice('მოუსმინე რიტმს. უფრო სწრაფი ორმაგი პულსი ნიშნავს, რომ უახლოვდები.');}} style={{backgroundColor:c.surface,borderColor:'#14B8A6',borderWidth:1,borderRadius:20,padding:13,flexDirection:'row',alignItems:'center',gap:12}}><Heart size={24} color="#14B8A6" fill={pulse.signal.revealed?'#14B8A6':'transparent'}/><View style={{flex:1}}><Copy bold size={13}>{pulse.signal.revealed?'აღმოჩენა შენ გვერდითაა':'რაღაც ახლოსაა…'}</Copy><Copy muted size={10}>{pulse.signal.revealed?'გახსენი კამერა და შეეხე ყუთს':'მიჰყევი პულსს · ხმა + ჰაპტიკა'}</Copy></View>{pulse.signal.revealed?<Gift color={c.primary100} size={22}/>:null}</Pressable>:null}
   <Card style={{padding:14,borderRadius:25,gap:12}}>
    <Pressable accessibilityRole="button" accessibilityLabel="გასეირნების დეტალები" onPress={()=>setDetails(true)} style={{flexDirection:'row',alignItems:'center',gap:12}}><View style={{flex:1}}><View style={{flexDirection:'row',gap:6,alignItems:'center'}}><Route size={13} color={c.primary100}/><Copy size={10} muted>გავლილი მანძილი</Copy></View><Copy bold size={24} style={{lineHeight:32}}>{formatDistanceShort(run.distanceM)}</Copy></View><View style={{flex:1}}><View style={{flexDirection:'row',gap:6,alignItems:'center'}}><Timer size={13} color={c.primary100}/><Copy size={10} muted>აქტიური დრო</Copy></View><Copy bold size={24} style={{lineHeight:32}}>{formatClock(run.movingMs)}</Copy></View><MoreHorizontal color={c.text300} size={20}/></Pressable>
    ...
    <View style={{flexDirection:'row',gap:9}}><View style={{flex:1}}><Action label={running?'პაუზა':run.phase==='paused'?'გაგრძელება':'დაწყება'} busy={busy} disabled={run.phase==='preparing'} icon={running?Pause:Play} onPress={()=>running?pauseRun():void begin()}/></View>{active?<IconButton label="სესიის დასრულება" icon={Flag} onPress={()=>{if(running)pauseRun();setFinish(true);}}/>:null}</View>
   </Card>
  </View>:null}
  ...
  <Sheet title="შენი მოძრაობის სივრცე" visible={menu} onClose={()=>setMenu(false)}>{([{id:'missions',label:'თბილისის პასპორტი',icon:Compass},{id:'collection',label:'ჩემი აღმოჩენები',icon:Gift},{id:'leaderboard',label:'ლიდერბორდი',icon:Trophy},{id:'settings',label:'პარამეტრები',icon:Settings2},{id:'help',label:'როგორ მუშაობს?',icon:BookOpen}] as const).map(item=><Action key={item.id} secondary label={item.label} icon={item.icon} onPress={()=>openPanel(item.id)}/>)}</Sheet>
