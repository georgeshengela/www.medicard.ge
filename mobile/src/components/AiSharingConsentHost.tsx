...
export function AiSharingConsentHost() {
  ...
  return <Modal visible {...APP_MODAL_PROPS} onRequestClose={close}>
    ...
    <View accessibilityViewIsModal style={{ width: '100%', maxWidth: 560, maxHeight: Math.max(240, height - insets.top - insets.bottom - 24), backgroundColor: colors.surface, borderRadius: 24, borderWidth: 1, borderColor: colors.bg300, overflow: 'hidden' }}>
      <View style={{ padding: 20, paddingBottom: 14, gap: 10, borderBottomWidth: 1, borderColor: colors.bg300 }}>
        <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
          ...
          <Pressable accessibilityRole="button" accessibilityLabel={prompt.status.accepted ? 'დახურვა — არჩევანის შეუცვლელად' : 'დახურვა — გაზიარების გარეშე'} disabled={prompt.busy} onPress={cancelAiSharingPrompt} style={{ minWidth: 44, minHeight: 44, alignItems: 'center', justifyContent: 'center' }}><X size={22} color={colors.text200} /></Pressable>
        </View>
        <Text accessibilityRole="header" style={{ color: colors.text100, fontFamily: 'NotoSansGeorgian_700Bold', fontSize: 20, lineHeight: 29 }}>{manifest.title}</Text>
        <Text style={[body, { fontSize: 12, lineHeight: 19 }]}>{prompt.status.accepted ? 'გაზიარება ნებადართულია. არჩევანის შეცვლა ნებისმიერ დროს შეგიძლია.' : 'შენი ნებართვის გარეშე გარე AI-ს მონაცემებს არ ვუგზავნით.'}</Text>
      </View>
      <ScrollView style={{ flexShrink: 1 }} keyboardShouldPersistTaps="handled" contentContainerStyle={{ padding: 20, gap: 14 }}>
        <View style={{ gap: 6 }}>
          <Text style={[body, { fontFamily: 'NotoSansGeorgian_700Bold', color: colors.text100 }]}>მონაცემების მიმღებები</Text>
          <Text style={body}>{manifest.recipients.map(provider => provider.name).join(' · ')}</Text>
        </View>
        ...
        {manifest.recipients.map(provider => <View key={provider.name} style={{ gap: 3 }}>
          <Pressable accessibilityRole="link" accessibilityLabel={`${provider.name} — კონფიდენციალურობის პოლიტიკა`} onPress={() => { void Linking.openURL(provider.url).catch(() => undefined); }} style={{ minHeight: 44, flexDirection: 'row', alignItems: 'center', gap: 8 }}>
        ...
        <Pressable accessibilityRole="button" accessibilityState={{ expanded: policyOpen }} onPress={() => setPolicyOpen(!policyOpen)} style={{ minHeight: 44, paddingVertical: 8 }}><Text style={[body, { color: colors.primary100, textDecorationLine: 'underline' }]}>{policyOpen ? 'პოლიტიკის შეკეცვა' : 'MEDICARD-ის კონფიდენციალურობის პოლიტიკა'}</Text></Pressable>
      ...
      <View style={{ padding: 16, borderTopWidth: 1, borderColor: colors.bg300, gap: 8 }}>
        ...
        <Pressable accessibilityRole="button" accessibilityState={{ disabled: prompt.busy, busy: prompt.busy }} disabled={prompt.busy} onPress={() => { void decideAiSharing(true); }} style={{ minHeight: 48, padding: 12, borderRadius: 14, backgroundColor: '#0F766E', opacity: prompt.busy ? 0.65 : 1, alignItems: 'center', justifyContent: 'center' }}>
          {prompt.busy ? <ActivityIndicator color="#FFFFFF" /> : <Text style={{ fontFamily: 'NotoSansGeorgian_600SemiBold', fontSize: 15, color: '#FFFFFF', textAlign: 'center' }}>{prompt.status.accepted ? 'თანხმობის შენარჩუნება' : 'ვეთანხმები AI-სთან გაზიარებას'}</Text>}
        </Pressable>
        <Pressable accessibilityRole="button" disabled={prompt.busy} onPress={() => { void decideAiSharing(false); }} style={{ minHeight: 46, padding: 10, alignItems: 'center', justifyContent: 'center' }}><Text style={[body, { color: colors.text100, textAlign: 'center' }]}>{prompt.status.accepted ? 'თანხმობის გაუქმება' : 'გაგრძელება AI-ის გარეშე'}</Text></Pressable>
      </View>
    </View>
  </View></Modal>;
}
