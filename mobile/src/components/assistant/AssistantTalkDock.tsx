...
export function AssistantTalkDock(props: {
  voice: boolean; voiceOutput: boolean; phase: VoicePhase; duration: number; metering?: number;
  ...
  const C = useThemeColors(), dark = useIsDark(), keyboardOpen = useChatKeyboardOpen();
  const [typing, setTyping] = useState(false), [tapMode, setTapMode] = useState(false), [screenReader, setScreenReader] = useState(false);
  ...
  const toggleMode = tapMode || screenReader;
  useEffect(() => {
    let current = true;
  ...
  return <ChatActionDock style={props.voiceStage ? { borderTopWidth: 0, backgroundColor: dark ? C.bg100 : '#FFFFFF' } : undefined}><View style={{ gap: 10 }}>
    {props.voice && !keyboardOpen && !typing ? <>
      <View style={{ alignItems: 'center', gap: 3 }}>
        <Text accessibilityLiveRegion="polite" style={{ color: C.text100, fontSize: 14, fontFamily: 'NotoSansGeorgian_700Bold' }}>{label}</Text>
      ...
      <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 30 }}>
        ...
        <View style={{ width: 118, height: 118, alignItems: 'center', justifyContent: 'center' }}>
          <Animated.View pointerEvents="none" style={{ position: 'absolute', width: 112, height: 112, borderRadius: 56, borderWidth: 1, borderColor: C.primary100, opacity: pulse.interpolate({ inputRange: [0, 1], outputRange: [.16, .32] }), transform: [{ scale: pulse.interpolate({ inputRange: [0, 1], outputRange: [1, 1.06] }) }] }} />
          <Pressable testID="assistant-talk-button" accessibilityRole="button" accessibilityLabel={toggleMode ? (recording ? 'ჩაწერის დასრულება და გაგზავნა' : 'საუბრის დაწყება') : 'დააჭირე და გააჩერე საუბრისთვის'}
            ...
            style={{ width: 96, height: 96, borderRadius: 48, alignItems: 'center', justifyContent: 'center', backgroundColor: recording ? '#BE123C' : '#0F766E', opacity: locked ? .62 : 1, borderWidth: 1, borderColor: recording ? '#E11D48' : '#0D9488' }}>
            {preparing || processing ? <ActivityIndicator color="#FFFFFF" size="large" /> : recording ? <View style={{ flexDirection: 'row', gap: 4, height: 35, alignItems: 'center' }}>{[.35, .65, 1, .75, .45].map((level, index) => <View key={index} style={{ width: 5, borderRadius: 3, height: 8 + level * Math.max(8, Math.min(27, ((props.metering ?? -30) + 60) * .7)), backgroundColor: '#FFFFFF' }} />)}</View> : <Mic size={34} strokeWidth={1.8} color="#FFFFFF" />}
          </Pressable>
        </View>
      ...
      <View style={{ flexDirection: 'row', justifyContent: 'center', gap: 12 }}>
        {!screenReader ? <Pressable accessibilityRole="button" disabled={props.phase !== 'idle' || !!props.busy} onPress={() => setTapMode(!tapMode)} style={{ minHeight: 32, justifyContent: 'center', paddingHorizontal: 10 }}><Text style={{ fontSize: 11, color: C.text200, fontFamily: 'NotoSansGeorgian_400Regular', textDecorationLine: 'underline' }}>{tapMode ? 'დაჭერით საუბარი' : 'ერთი შეხებით ჩაწერა'}</Text></Pressable> : null}
      </View>
    </> : <View style={{ gap: 8 }}>
      ...
      <View style={{ flexDirection: 'row', gap: 8, alignItems: 'flex-end' }}>
        ...
        <TextInput ref={input} accessibilityLabel="შეტყობინება Medi-სთვის" value={props.text} onChangeText={props.onText} editable={!processing} multiline maxLength={4000} placeholder="რას გავაკეთებთ დღეს?" placeholderTextColor={C.text200}
      ...
      <Text style={{ color: C.text200, fontSize: 11, lineHeight: 16, fontFamily: 'NotoSansGeorgian_400Regular' }}>შენახვამდე Medi დეტალებს შეგითანხმებს.</Text>
    </View>}
  </View></ChatActionDock>;
}
