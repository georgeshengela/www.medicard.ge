...
const VARIANTS: Record<WeatherLang, Record<string, CopyPair[]>> = {
  ka: {
    excellent_outdoor: [
      ...
      { title: 'დღეს ამინდი შენს მხარესაა 😄', body: 'ცოტა სუფთა ჰაერი ცუდი იდეა ნამდვილად არ არის.' },
      { title: 'სასიამოვნო დღეა გარეთ გასასვლელად.', body: 'Medi-ს ხმა ამბობს: ცოტა გავიაროთ? 🚶' },
...
export const WEATHER_UI: Record<WeatherLang, {
  ...
  ka: {
    ...
    feelsLike: (n) => `იგრძნობა როგორც ${Math.round(n)}°`,
