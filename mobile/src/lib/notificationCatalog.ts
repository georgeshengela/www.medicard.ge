...
export function notificationCatalog(): NotificationCatalogItem[] {
  return [
    ...
    item({
      id: 'pregnancy-care-plan',
      group: 'cycle',
      send: 'local',
      label: 'დაგეგმილი მოვლის შეხსენება',
      how: 'Local · DATE 09:00 + quiet bump. მხოლოდ user plannedDate + explicit opt-in. Brain revalidate.',
      channelId: CYCLE_CHANNEL_ID,
      data: {
        ...
        route: '/cycle/pregnancy/care-plan?item=anatomy_ultrasound',
      },
    }),
    item({
      id: 'pregnancy-care-masked',
      group: 'cycle',
      send: 'local',
      label: 'დაგეგმილი მოვლა · დისკრეტული',
      how: 'იგივე pregnancy_care DATE, lock-screen ტექსტი pregnancy-care-masked.',
      channelId: CYCLE_DISCREET_CHANNEL_ID,
      data: {
        type: 'pregnancy_care_plan',
        family: 'pregnancyCareReminder',
        masked: true,
        route: '/cycle/pregnancy/care-plan',
      },
    }),
    item({
      id: 'visit',
