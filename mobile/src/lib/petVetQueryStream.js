...
function throwHttpError(status, text, retryAfter) {
  const payload = text
    ...
    : {};
  const message = (typeof payload?.error === 'string' && payload.error) || `${ka.common.error} (${status})`;
...
function applyEvents(events, onDelta) {
  ...
  for (const event of events) {
    if (event?.type === 'delta' && event.text) {
      ...
      throw new ApiError(event.error || ka.common.error, Number(event.status) || 502, {
        code: event.code || 'AI_ENGINE_ERROR',
      });
    }
  }
  return done;
...
/** Streams POST /api/pets/:petId/chat/query. Never sends human DOCTOR/CONSILIUM. */
export async function streamPetVetQuery(petId, body, { onDelta, signal } = {}) {
  try {
    ...
    if (!done) {
      throw new ApiError(ka.pets.vetPartial, 502, { code: 'PARTIAL' });
    }
    return done;
  } catch (error) {
    if (error instanceof ApiError) throw error;
    if (error?.name === 'AbortError') throw new ApiError(ka.pets.vetCancelled, 499, { code: 'CANCELLED' });
