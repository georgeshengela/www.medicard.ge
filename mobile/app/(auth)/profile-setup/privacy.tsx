    ...
    <View style={{ flex: 1, backgroundColor: '#FFFFFF', paddingTop: welcomeTopInset(insets.top) }}>
      <ScrollView contentContainerStyle={{ paddingBottom: 24 }} showsVerticalScrollIndicator={false}>
        <View style={{ alignItems: 'center', paddingHorizontal: 16, paddingVertical: 24, gap: 16 }}>
          ...
          <View style={{ paddingHorizontal: 8, paddingVertical: 4, borderRadius: 8, borderWidth: 1, borderColor: '#D1D5DB', backgroundColor: '#F9FAFB' }}>
            <Text style={{ fontFamily: 'NotoSansGeorgian_500Medium', fontSize: 12, color: '#1F2937' }}>v{version}</Text>
          </View>
          <Text style={{ fontFamily: 'NotoSansGeorgian_700Bold', fontSize: 30, lineHeight: 38, color: '#1F2937', textAlign: 'center' }}>
            {ka.profileSetup.privacyTitle}
          </Text>
          <Text style={{ fontFamily: 'NotoSansGeorgian_400Regular', fontSize: 18, color: '#4B5563' }}>
            {ka.profileSetup.privacyEffective('25 აგვ. 2026')}
          </Text>
          <Text style={{ fontFamily: 'NotoSansGeorgian_400Regular', fontSize: 14, lineHeight: 22, color: '#4B5563', textAlign: 'center' }}>
            {ka.profileSetup.privacyIntro}
          </Text>
        </View>

        {SECTIONS.map((section) => (
          <View key={section.title} style={{ paddingHorizontal: 16, paddingVertical: 20, borderTopWidth: 1, borderTopColor: '#E5E7EB' }}>
            <Text style={{ fontFamily: 'NotoSansGeorgian_700Bold', fontSize: 20, color: '#1F2937', marginBottom: 12 }}>
              {section.title}
            </Text>
            {section.paragraphs?.map((p) => (
              <Text key={p} style={{ fontFamily: 'NotoSansGeorgian_400Regular', fontSize: 14, lineHeight: 22, color: '#4B5563', marginBottom: 8 }}>
                {p}
              </Text>
            ))}
            {section.bullets?.map((b) => (
              <Text key={b} style={{ fontFamily: 'NotoSansGeorgian_400Regular', fontSize: 14, lineHeight: 22, color: '#4B5563', marginBottom: 6, paddingLeft: 8 }}>
                {'\u2022 '}{b}
              </Text>
            ))}
          </View>
      ...
      <View style={{ paddingHorizontal: 16, paddingBottom: Math.max(insets.bottom, 16), gap: 12 }}>
        <ProfileSetupPrimaryButton label={ka.profileSetup.privacyAccept} onPress={() => void accept()} loading={busy} icon="check" />
