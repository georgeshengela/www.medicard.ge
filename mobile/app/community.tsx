...
function TopicIcon({topic,color,size=16}:{topic:string;color:string;size?:number}){const Icon=topicIcons[topic]||Heart;return <Icon size={size} color={color} strokeWidth={1.6}/>;}
...
function CommunityPhoto({id}:{id:string}){
 ...
 return uri?<Image accessibilityLabel="პოსტის ფოტო" source={{uri}} style={{width:'100%',aspectRatio:1.25,borderRadius:0,backgroundColor:c.bg200}} resizeMode="cover"/>:<Pressable accessibilityRole="button" accessibilityLabel="ფოტოს ხელახლა ჩატვირთვა" onPress={()=>setRetry(n=>n+1)} style={{height:160,borderRadius:0,backgroundColor:c.bg200,alignItems:'center',justifyContent:'center'}}>{failed?<Text style={{color:c.text200}}>ფოტო ვერ ჩაიტვირთა · სცადე ხელახლა</Text>:<ActivityIndicator color={c.primary100}/>}</Pressable>;
}
function Space({eligible}:{eligible:boolean}){
 ...
 const loadFeed=useCallback(async(more=false,silent=false)=>{
  ...
  try{const result=await call<{posts:Content[];next:string|null}>(`/posts?topic=${topic}&mine=${mine}&limit=${silent?Math.min(feedCount.current,100):20}${more&&next?'&before='+encodeURIComponent(next):''}`);if(live.current&&gen===feedGen.current){setError('');setPosts(p=>more?[...p,...result.posts.filter(x=>!p.some(y=>y.id===x.id))]:result.posts);setNext(result.next);}}
  catch(e){if(live.current&&gen===feedGen.current){if(!silent||[401,403].includes((e as {status:number}).status))setError((e as Error).message);if([401,403].includes((e as {status:number}).status)){setPosts([]);setComments([]);setSelected(null);setMember(null);setReady(false);}}}finally{if(live.current&&gen===feedGen.current)setLoading(false);}
 },[topic,mine,next]);
 ...
 const avatar=(anon:boolean,alias='')=> anon?<AnonymousAvatar/>:<View style={{width:34,height:34,borderRadius:17,backgroundColor:c.accent100,alignItems:'center',justifyContent:'center'}}>{text((alias||'მ').slice(0,1).toUpperCase(),17,c.primary100)}</View>;
 const card=(item:Content,detail=false)=> <View style={{paddingHorizontal:18,paddingTop:16,paddingBottom:6,gap:10}}>
  ...
  {item.topic&&<View style={{flexDirection:'row',alignItems:'center',gap:6}}><TopicIcon topic={item.topic} color={c.primary100} size={13}/>{text(topics.find(t=>t[0]===item.topic)?.[1]||'',11,c.text200)}</View>}
  ...
  {item.hasImage&&<View style={{marginHorizontal:-18}}><CommunityPhoto key={item.id+':'+item.revision} id={item.id}/></View>}
 ...
 return <KeyboardAvoidingView enabled={!!member} style={{flex:1,width:'100%',maxWidth:760,alignSelf:'center',backgroundColor:c.bg100,paddingTop:safe.top}} behavior={Platform.OS==='ios'?'padding':'height'}>
  ...
  />:
  page==='feed'?<FlatList data={posts} keyExtractor={p=>p.id} renderItem={({item})=>card(item)} ItemSeparatorComponent={()=> <View style={{height:1,backgroundColor:c.bg300}}/>} contentContainerStyle={{paddingBottom:safe.bottom+24}} refreshControl={<RefreshControl refreshing={loading} onRefresh={()=>void loadFeed()} tintColor={c.primary100}/>}
   ListHeaderComponent={<View>
    <View style={{paddingHorizontal:18,paddingVertical:16,flexDirection:'row',alignItems:'center',gap:12,borderBottomWidth:1,borderColor:c.bg300}}>
     {avatar(false,member.alias)}
     <Pressable accessibilityRole="button" accessibilityLabel="ახალი პოსტის დაწერა" onPress={compose} style={{flex:1,minHeight:44,justifyContent:'center',gap:3}}>{text('რას გაუზიარებ დღეს?',14,c.text200)}{text('მეტსახელით ან ანონიმურად',10,c.text200)}</Pressable>
     <Pressable accessibilityRole="button" accessibilityLabel="პოსტის ფოტოს დამატება" onPress={()=>{compose();choosePhoto();}} style={{width:44,height:44,borderRadius:14,backgroundColor:c.accent100,alignItems:'center',justifyContent:'center'}}><ImagePlus size={21} strokeWidth={1.6} color={c.primary100}/></Pressable>
    </View>
    <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={{paddingHorizontal:18,gap:22}} style={{borderBottomWidth:1,borderColor:c.bg300}}>{topics.map(([key,label])=><Pressable key={key} accessibilityRole="tab" accessibilityState={{selected:topic===key}} onPress={()=>setTopic(key)} style={{minHeight:48,flexDirection:'row',alignItems:'center',gap:6,borderBottomWidth:topic===key?2:0,borderColor:c.primary100}}><TopicIcon topic={key} color={topic===key?c.primary100:c.text200}/><Text style={{fontFamily:topic===key?'NotoSansGeorgian_600SemiBold':'NotoSansGeorgian_400Regular',fontSize:12,color:topic===key?c.text100:c.text200}}>{label}</Text></Pressable>)}</ScrollView>
    <View style={{paddingHorizontal:18,paddingTop:14,flexDirection:'row',alignItems:'center',justifyContent:'space-between'}}><View style={{flexDirection:'row',alignItems:'center',gap:6}}><MessagesSquare size={14} strokeWidth={1.5} color={c.text200}/>{text(mine?'შენი ჩანაწერები':'ბოლო ამბები',11,c.text200)}</View><Pressable accessibilityRole="button" accessibilityState={{selected:mine}} onPress={()=>setMine(!mine)} style={{minHeight:44,flexDirection:'row',alignItems:'center',gap:6}}><CircleUserRound size={16} strokeWidth={1.6} color={c.primary100}/>{text(mine?'ყველას ნახვა':'ჩემი პოსტები',11,c.primary100)}</Pressable></View>
   </View>}
  ...
  page==='compose'?<View style={{flex:1}}><ScrollView keyboardShouldPersistTaps="handled" contentContainerStyle={{padding:18,gap:16,paddingBottom:safe.bottom+20}}>{anonymity}<TextInput accessibilityLabel="პოსტის ტექსტი" value={body} onChangeText={setBody} multiline maxLength={3000} placeholder="რისი გაზიარება გინდა?" placeholderTextColor={c.text200} style={[inputStyle,{minHeight:200,borderWidth:0,borderRadius:0,backgroundColor:'transparent',paddingHorizontal:0,textAlignVertical:'top',fontSize:16,lineHeight:26}]}/>{text(`${body.length}/3000`,11,c.text200)}<ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={{gap:8}}>{topics.slice(1).map(([key,label])=><View key={key}>{button(label,()=>setDraftTopic(key),topicIcons[key],draftTopic===key)}</View>)}</ScrollView>{photo?<View><Image source={{uri:photo.uri}} style={{width:'100%',height:210,borderRadius:20}}/>{button('ფოტოს მოშორება',()=>setPhoto(null),X)}</View>:button('ფოტოს დამატება',choosePhoto,ImagePlus)}{text('პოსტი მაშინვე გამოჩნდება სივრცეში. ფოტოდან მდებარეობის ინფორმაცია ავტომატურად იშლება.',12,c.text200)}</ScrollView><View style={{paddingHorizontal:18,paddingTop:10,paddingBottom:Math.max(safe.bottom,12),borderTopWidth:1,borderColor:c.bg300,backgroundColor:c.surface}}>{button(editing?'ცვლილებების შენახვა':'გამოქვეყნება',submit,Send,true,!body.trim())}</View></View>:
  page==='detail'&&selected?<View style={{flex:1}}>
   <ScrollView ref={detailScroll} keyboardShouldPersistTaps="handled" keyboardDismissMode={Platform.OS==='ios'?'interactive':'on-drag'} contentContainerStyle={{gap:18,paddingBottom:20}}>
    {card(selected,true)}
    <View style={{paddingHorizontal:18,flexDirection:'row',alignItems:'center',gap:8}}><MessageCircle size={19} color={c.primary100}/>{text('საუბარი · '+selected.comments,18)}</View>
    {communityThreads(comments).map(({item,depth})=><View key={item.id} style={{marginHorizontal:18,marginLeft:18+depth*12,borderLeftWidth:item.parentId?2:0,borderLeftColor:c.bg300,paddingLeft:item.parentId?12:0,gap:8}}>
