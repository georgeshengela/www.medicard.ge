import {distance} from './engine.ts';
import type {Coordinate} from './engine.ts';
import type {Journey} from './journey.ts';

export type Chapter='green'|'oldtown'|'horizon'|'culture';
export type Mission={id:string;name:string;chapter:Chapter;center:Coordinate;radius:number;meters:number;seconds:number;title:string;story:string;tip:string;icon:'trees'|'landmark'|'waves'|'mountain'|'bridge'|'flower';source:string};
export const CHAPTERS=[{id:'green' as const,name:'მწვანე თბილისი',subtitle:'ქალაქის მშვიდი მხარე',symbol:'01'},{id:'oldtown' as const,name:'ძველი ქალაქის ამბები',subtitle:'ერთი ადგილი. ახალი ისტორია.',symbol:'02'},{id:'horizon' as const,name:'წყალი და ჰორიზონტი',subtitle:'მეტი სივრცე შენი ნაბიჯებისთვის',symbol:'03'},{id:'culture' as const,name:'ქალაქის ხელწერა',subtitle:'აღმოაჩინე თბილისის ხასიათი',symbol:'04'}];
const locationSource=(lat:number,lon:number)=>`https://www.openstreetmap.org/?mlat=${lat}&mlon=${lon}#map=17/${lat}/${lon}`;
const make=(id:string,name:string,chapter:Chapter,center:Coordinate,radius:number,meters:number,title:string,story:string,icon:Mission['icon'],tip='იარე საჯარო საფეხმავლო სივრცეში. ზონის ცენტრში ზუსტად მისვლა საჭირო არ არის.',source=locationSource(center[1],center[0])):Mission=>({id,name,chapter,center,radius,meters,seconds:60,title,story,icon,tip,source});
...
export type MissionProgress={meters:number;seconds:number;completedAt:string|null};
export type MissionBook={version:1;selected:string|null;progress:Record<string,MissionProgress>};
export const emptyBook=():MissionBook=>({version:1,selected:null,progress:{}});
export const missionById=(id:string|null)=>MISSIONS.find(m=>m.id===id);
export const missionProgress=(book:MissionBook,m:Mission):MissionProgress=>book.progress[m.id]||{meters:0,seconds:0,completedAt:null};
export function missionPercent(book:MissionBook,m:Mission){const p=missionProgress(book,m);return p.completedAt?100:Math.floor(Math.min(p.meters/m.meters,p.seconds/m.seconds,1)*100);}
export function loadBook(raw:unknown):MissionBook{const book=emptyBook();if(!raw||typeof raw!=='object')return book;const d=raw as MissionBook;if(d.version!==1)return book;book.selected=missionById(d.selected)?.id||null;for(const m of MISSIONS){const p=d.progress?.[m.id];if(!p)continue;const num=(n:number,max:number)=>Number.isFinite(n)?Math.min(max,Math.max(0,n)):0;const meters=num(p.meters,m.meters),seconds=num(p.seconds,m.seconds);book.progress[m.id]={meters,seconds,completedAt:meters>=m.meters&&seconds>=m.seconds&&typeof p.completedAt==='string'&&!Number.isNaN(Date.parse(p.completedAt))?p.completedAt:null};}return book;}
/** Count only accepted movement inside the selected mission. Pauses and teleports contribute zero. */
export function advanceMission(book:MissionBook,before:Journey,after:Journey,now=Date.now(),mission:Mission|null|undefined=missionById(book.selected)):MissionBook{
 const m=mission;if(!m||before.source!==after.source||missionProgress(book,m).completedAt)return book;
 const meters=after.meters-before.meters,seconds=after.movingSeconds-before.movingSeconds;
 if(meters<=0||seconds<=0||seconds>15||meters/seconds>7||after.accuracy>25||before.accuracy>25||!['tracking','off-path'].includes(after.status))return book;
 if(after.source==='gps'&&(after.lastFix===null||now-after.lastFix>15000||before.lastFix===null))return book;
 if(distance(before.position,after.position)>meters+20)return book;
 if(distance(before.position,m.center)+before.accuracy>m.radius||distance(after.position,m.center)+after.accuracy>m.radius)return book;
 const p=missionProgress(book,m),next={meters:Math.min(m.meters,p.meters+meters),seconds:Math.min(m.seconds,p.seconds+seconds),completedAt:null as string|null};
 if(next.meters>=m.meters&&next.seconds>=m.seconds)next.completedAt=new Date(now).toISOString();
 return {...book,progress:{...book.progress,[m.id]:next}};
}
export function missionCircle(m:Mission){const coordinates:Coordinate[]=[];for(let i=0;i<=64;i++){const a=i/64*Math.PI*2;coordinates.push([m.center[0]+Math.cos(a)*m.radius/(111195*Math.cos(m.center[1]*Math.PI/180)),m.center[1]+Math.sin(a)*m.radius/111195]);}return {type:'Feature' as const,properties:{},geometry:{type:'Polygon' as const,coordinates:[coordinates]}};}
