/**
 * Phase 37 Android capture helper.
 * Usage: node qa/cycle-phase37-visit-place/qa-run.js dump|shot name|tap regex|coord x y|font 1.0|night yes|size 1080x2400|open cycle|events|grant
 */
...
function dump() {
  try {
    sh(`adb -s ${serial} shell uiautomator dump /sdcard/ui-p37.xml`);
  } catch {
    /* ignore */
  }
  const xmlPath = path.join(dir, '_live.xml');
  sh(`adb -s ${serial} pull /sdcard/ui-p37.xml "${xmlPath}"`);
...
if (action === 'dump') {
  ...
  process.exit(ok ? 0 : 1);
} else if (action === 'anatomy') {
  const { nodes } = dump();
  const hit = nodes.find((n) => /დაგეგმილი, 2026-09-22/.test(`${n.t} ${n.d}`) && n.y < 2100)
    || nodes.find((n) => /ანატომიის ულტრაბგერა, ხშირად/.test(`${n.t} ${n.d}`));
  if (!hit) {
    console.error('NOT_FOUND anatomy');
    process.exit(2);
  }
  console.log('TAP', hit.t || hit.d, hit.x, Math.min(hit.y, 2000));
  tap(hit.x, Math.min(hit.y, 2000));
} else if (action === 'open') {
  const dest = process.argv[3] || 'cycle';
  sh(`adb -s ${serial} shell am start -a android.intent.action.VIEW -d "exp://192.168.1.104:8081/--/${dest}"`);
  ...
  const out = sh(
    `adb -s ${serial} shell content query --uri content://com.android.calendar/events --projection _id:title:dtstart:dtend:allDay:hasAlarm:description:eventLocation`,
  );
  ...
  const needle = new RegExp(process.argv[3] || 'ვიზიტის ადგილი');
