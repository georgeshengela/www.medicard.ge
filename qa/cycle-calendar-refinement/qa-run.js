...
function dump() {
  try {
    sh(`adb -s ${serial} shell uiautomator dump /sdcard/ui-cal.xml`);
  } catch {
    /* ignore */
  }
  const xmlPath = path.join(dir, '_live.xml');
  sh(`adb -s ${serial} pull /sdcard/ui-cal.xml "${xmlPath}"`);
...
if (action === 'dump') {
  const { nodes } = dump();
  const interesting = nodes.filter((n) => n.t || n.d).slice(0, 120);
  ...
  console.error('dump | shot name | tap regex | coord x y | key code');
  process.exit(1);
}
