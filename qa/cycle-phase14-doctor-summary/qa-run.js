/**
 * Phase 14 Android capture helper.
 * Usage: node qa/cycle-phase14-doctor-summary/qa-run.js dump|shot name|tap regex|coord x y|cycle|summary|reload|font 1.0|night yes|size 1080x2400|swipe ...
 */
...
function dump() {
  try {
    sh(`adb -s ${serial} shell uiautomator dump /sdcard/ui-p14.xml`);
  } catch {
    /* ignore */
  }
  const xmlPath = path.join(dir, '_live.xml');
  sh(`adb -s ${serial} pull /sdcard/ui-p14.xml "${xmlPath}"`);
...
if (action === 'dump') {
  const { nodes } = dump();
  console.log(JSON.stringify(nodes.filter((n) => n.t || n.d).slice(0, 140), null, 2));
} else if (action === 'shot') {
  shot(process.argv[3]);
} else if (action === 'tap') {
  const { nodes } = dump();
  ...
  console.error('dump | shot name | tap regex | coord x y | cycle | summary | reload | font n | night yes|no | size WxH | swipe x1 y1 x2 y2 ms');
  process.exit(1);
}
