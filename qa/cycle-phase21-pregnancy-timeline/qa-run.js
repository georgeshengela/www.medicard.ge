/**
 * Phase 21 Android capture helper.
 * Usage: node qa/cycle-phase21-pregnancy-timeline/qa-run.js dump|shot name|tap regex|coord x y|reload|font 1.0|night yes|size 1080x2400|open timeline
 */
...
function dump() {
  try {
    sh(`adb -s ${serial} shell uiautomator dump /sdcard/ui-p21.xml`);
  } catch {
    /* ignore */
  }
  const xmlPath = path.join(dir, '_live.xml');
  sh(`adb -s ${serial} pull /sdcard/ui-p21.xml "${xmlPath}"`);
...
if (action === 'dump') {
  ...
  const ok = waitFor(process.argv[3] || 'ორსულობის', Number(process.argv[4]) || 40000);
  console.log(ok ? 'READY' : 'TIMEOUT');
  process.exit(ok ? 0 : 1);
} else if (action === 'open') {
  const dest = process.argv[3] === 'timeline' ? '/cycle/pregnancy/timeline' : '/cycle';
  ...
  console.log('dump|shot name|tap regex|coord x y|font 1.0|night yes|size 1080x2400|open timeline|swipe');
}
