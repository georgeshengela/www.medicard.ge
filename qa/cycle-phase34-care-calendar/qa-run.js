/**
 * Phase 34 Android capture helper.
 * Usage: node qa/cycle-phase34-care-calendar/qa-run.js dump|shot name|tap regex|coord x y|font 1.0|night yes|size 1080x2400|open cycle|events|grant
 */
...
function dump() {
  try {
    sh(`adb -s ${serial} shell uiautomator dump /sdcard/ui-p34.xml`);
  } catch {
    /* ignore */
  }
  const xmlPath = path.join(dir, '_live.xml');
  sh(`adb -s ${serial} pull /sdcard/ui-p34.xml "${xmlPath}"`);
...
if (action === 'dump') {
  ...
  for (const pkg of pkgs) {
    try {
      sh(`adb -s ${serial} shell pm grant ${pkg} android.permission.READ_CALENDAR`);
      sh(`adb -s ${serial} shell pm grant ${pkg} android.permission.WRITE_CALENDAR`);
  ...
  const needle = new RegExp(process.argv[3] || 'კალენდარში დამატება');
  const timeout = Number(process.argv[4]) || 18000;
  const dir = process.argv[5] === 'up' ? 'up' : 'down';
  const t0 = Date.now();
  let ok = false;
  while (Date.now() - t0 < timeout) {
    const { xml } = dump();
