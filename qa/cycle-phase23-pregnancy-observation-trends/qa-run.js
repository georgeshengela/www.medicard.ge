/**
 * Phase 23 Android capture helper.
 * Usage: node qa/cycle-phase23-pregnancy-observation-trends/qa-run.js dump|shot name|tap regex|coord x y|font 1.0|night yes|size 1080x2400|open journal|swipe
 */
...
function dump() {
  try {
    sh(`adb -s ${serial} shell uiautomator dump /sdcard/ui-p23.xml`);
  } catch {
    /* ignore */
  }
  const xmlPath = path.join(dir, '_live.xml');
  sh(`adb -s ${serial} pull /sdcard/ui-p23.xml "${xmlPath}"`);
