/**
 * Phase 22 Android capture helper.
 * Usage: node qa/cycle-phase22-pregnancy-observations/qa-run.js dump|shot name|tap regex|coord x y|reload|font 1.0|night yes|size 1080x2400|open cycle|swipe
 */
...
function dump() {
  try {
    sh(`adb -s ${serial} shell uiautomator dump /sdcard/ui-p22.xml`);
  } catch {
    /* ignore */
  }
  const xmlPath = path.join(dir, '_live.xml');
  sh(`adb -s ${serial} pull /sdcard/ui-p22.xml "${xmlPath}"`);
...
if (action === 'dump') {
  ...
  const dest = process.argv[3] === 'journal' ? '/cycle?tab=journal' : '/cycle';
  ...
  console.log('dump|shot name|tap regex|coord x y|font 1.0|night yes|size 1080x2400|open cycle|swipe');
}
