/**
 * Phase 39 Android capture helper.
 * Usage: node qa/cycle-phase39-postpartum-doctor/qa-run.js dump|shot name|tap regex|coord x y|font 1.0|night yes|size 1080x2400|open summary|swipe
 */
...
function dump() {
  try {
    sh(`adb -s ${serial} shell uiautomator dump /sdcard/ui-p39.xml`);
  } catch {
    /* ignore */
  }
  const xmlPath = path.join(dir, '_live.xml');
  sh(`adb -s ${serial} pull /sdcard/ui-p39.xml "${xmlPath}"`);
...
if (action === 'dump') {
  const { nodes } = dump();
  console.log(JSON.stringify(nodes.filter((n) => n.t).slice(0, 220), null, 2));
} else if (action === 'shot') {
  shot(process.argv[3]);
} else if (action === 'tap') {
  const { nodes } = dump();
  ...
  const needle = new RegExp(process.argv[3] || 'შეჯამება');
  let ok = false;
  ...
  const dest =
    process.argv[3] === 'summary'
      ? '/cycle/summary'
      : process.argv[3] === 'settings'
        ? '/cycle/settings'
        : process.argv[3] === 'pregnancy'
          ? '/cycle/pregnancy'
          : '/cycle';
  sh(`adb -s ${serial} shell am start -a android.intent.action.VIEW -d "exp://10.0.2.2:8081/--${dest}"`);
} else if (action === 'reopen') {
  const dest = process.argv[3] === 'summary' ? '/cycle/summary' : '/cycle';
  ...
  console.log('dump|shot name|tap regex|coord x y|font 1.0|night yes|size 1080x2400|open summary');
}
