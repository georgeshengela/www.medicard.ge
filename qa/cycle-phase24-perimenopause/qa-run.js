/**
 * Phase 24 Android capture helper.
 * Usage: node qa/cycle-phase24-perimenopause/qa-run.js dump|shot name|tap regex|coord x y|font 1.0|night yes|size 1080x2400|open journal|swipe
 */
...
function dump() {
  try {
    sh(`adb -s ${serial} shell uiautomator dump /sdcard/ui-p24.xml`);
  } catch {
    /* ignore */
  }
  const xmlPath = path.join(dir, '_live.xml');
  sh(`adb -s ${serial} pull /sdcard/ui-p24.xml "${xmlPath}"`);
...
if (action === 'dump') {
  ...
  const needle = new RegExp(process.argv[3] || 'პერიმენოპაუზ');
  let ok = false;
  ...
  const needle = new RegExp(process.argv[3] || 'ციკლის ცვალებადობა');
  ...
  console.log('dump|shot name|tap regex|coord x y|font 1.0|night yes|size 1080x2400|open journal|swipe');
}
